...
Application::Application(const std::string &appName)
{
  ...
  m_window =
      std::unique_ptr<Window>(Window::CreateWindow(WindowInformation(appName)));
  ...
  Renderer::Init({"assets/shaders/2DShader.vertex.glsl",
...
{
  RenderCommand::ResizeRenderer(windowResizeEvent.GetWidth(),
...
void Application::AddLayer(Layer *layer) { m_layerLists.AddLayer(layer); }
void Application::RemoveLayer(Layer *layer) { m_layerLists.RemoveLayer(layer); }
