...
glTFModel::glTFModel(const std::string &path) : m_filePath(path)
{
...
void glTFModel::Draw(std::shared_ptr<Shader> &shader, Camera &camera)
{
  ...
  for(uint32_t i = 0; i < m_meshes.size(); i++)
  {
    ...
    m_meshes[i].Draw(m_modelMatrices[i]);
  }
}
...
void glTFModel::treverseNode(uint32_t node, glm::mat4 nodeMatrix)
{
  ...
  glm::mat4 nextNodeMatrix =
      nodeMatrix * matrix * positionMat * rotationMat * scaleMat;
...
std::vector<float> glTFModel::getFloats(json accessor)
{
  ...
  if(accessorType == "SCALAR")
    ...
    ASSERT(true, "Unknown ValueType Spesified While Fetching Accessor Type in "
                 "glTFModel: " +
                     m_filePath);
  ...
  {
    // we check the spesific index of the m_data char for the byte value in the
    // bin file and convert it into a value for us to access
    unsigned char bytes[] = {m_data[i++], m_data[i++], m_data[i++],
                             m_data[i++]};
    float byteValue;
...
std::vector<uint32_t> glTFModel::getIndices(json accessor)
{
  ...
  {
  case 5125: // uint32_t
    for(uint32_t i = dataBegin;
        i < dataBegin + accessorValuesCount * sizeof(uint32_t);)
    {
      unsigned char bytes[] = {m_data[i++], m_data[i++], m_data[i++],
                               m_data[i++]};
      uint32_t byteValue;
  ...
  case 5123:
    for(uint32_t i = dataBegin;
        i < dataBegin + accessorValuesCount * sizeof(uint16_t);)
    {
  ...
  case 5122:
    for(uint32_t i = dataBegin;
        i < dataBegin + accessorValuesCount * sizeof(int16_t);)
    {
      unsigned char bytes[] = {m_data[i++], m_data[i++]};
      int16_t byteValue;
    ...
    break;
  default:
    ASSERT(true, "Unknown ValueType While Fetching Accessor Component Type In "
                 "glTFModel: " +
                     m_filePath);
  }
...
std::vector<Mesh> glTFModel::loadMeshes(uint32_t meshIndex)
{
  ...
  std::vector<float> texCoords =
      getFloats(m_JSON["accessors"][textureCoordIndex]);
    ...
    const std::vector<glm::vec3> &normals)
{
  ...
  {
    vertices.push_back(
        {positions[i], glm::vec3(1.0f, 1.0f, 1.0f), texCoords[i], normals[i]});
  }
  return vertices;
}